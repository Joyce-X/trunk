...
//  UIImage *launchImage = [UIImage imageNamed:launchImageName];
    UIImage *launchImage = [UIImage imageNamed:@"defaultImage.jpg"];
...
//  }
  ...
  if (showSkipButton) {
    ...
    if (showCountTimeOfButton) {
      ...
      width = 65;
      [skipButton setTitle:JJLocalizedString(@"跳过广告", nil) forState:UIControlStateNormal];
    }
    skipButton.frame =
...
- (void)autoCount {
  if (self.adTime == 0) {
    ...
    NSString *title = JJLocalizedString(@"跳过广告", nil);
  if (self.skipBtn && self.showCountTimeOfButton) {
    if (self.adTime == 1) {
      [self.skipBtn
          setTitle:[NSString stringWithFormat:@"%@  %d",title, self.adTime]
