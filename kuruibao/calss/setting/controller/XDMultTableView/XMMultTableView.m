...
#define DEFAULTHEADVIEWCOLOR [UIColor colorWithRed:247/255.f green:247/255.f blue:247/255.f alpha:1]

@interface XMMultTableView()
@property (nonatomic, strong, readwrite) NSMutableArray *multopenSectionArray;

@end

@implementation XMMultTableView
...
- (void)tapHeader:(UITapGestureRecognizer *) tap {
    ...
    BOOL willOpen = [_multopenSectionArray containsObject:sectionObj];
    if (willOpen) {
...
//        [_tableView insertRowsAtIndexPaths:insertArray withRowAnimation:UITableViewRowAnimationFade];
    ...
    [self.delegate mTableView:self sectionHeaderDidClick:section willOpen:!willOpen];
}
...
- (NSInteger)invoke_numberOfSections{
    if (self.datasource&&[self.datasource respondsToSelector:@selector(numberOfSectionsInMTableView:)]) {
        return [self.datasource numberOfSectionsInMTableView:self];
    }
    return 1;
...
- (void)updateData:(NSArray *)seriseArray section:(NSInteger)section willOpen:(BOOL)willOpen
{
    ...
    for (int i = 0; i < seriseArray.count; i++)
    {
