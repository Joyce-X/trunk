...
+ (NSArray<JJGPSModel *> *)handelCoordinate:(NSArray *)array
{
    ...
    for (NSDictionary *dic in array)
    {
        ...
        model.speed = [newDic[@"speed"] integerValue];
        model.eventtype = [newDic[@"eventtype"] integerValue];
        model.eventstatus = newDic[@"eventstatus"];
        ...
        //!< 判断是否在中国境内
        if ([self outOfChina:latitude bdLon:longitude])
        {
            //!< 不在中国，直接使用wgs
              desCoor = coor;
         }else
        {
            //!< 在中国，转换为国测局坐标  wgs->gcj-02
             desCoor = [self wgs84ToGcj02:coor];
         }
         CLLocation *location = [[CLLocation alloc]initWithLatitude:desCoor.latitude longitude:desCoor.longitude];
...
+ (NSArray<JJGPSModel *> *)handelCoordinateForRealTimeMonitor:(NSArray *)array
{
    ...
    for (NSDictionary *dic in array)
    {
        ...
        model.dthappen = newDic[@"dthappen"];
        
        if(latitude == 0 && longitude == 0)
        {
            //!<取出错误的数据
            XMLOG(@"---------实时监控返回坐标有为0的时候，已经跳过---------");
            
            [XMMike addLogs:@"---------实时监控返回坐标有为0的时候，已经跳过---------"];
            continue;
